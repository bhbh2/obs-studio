...
class TrendFollowingStrategy(Strategy):
    ...
    def generate_signal(self, data):
        if len(data) < self.long_window:
            return 0
        short_ma = data['price'].rolling(window=self.short_window).mean().iloc[-1]
        long_ma = data['price'].rolling(window=self.long_window).mean().iloc[-1]
        if short_ma > long_ma:
            return 1  # 买入信号
...
class MeanReversionStrategy(Strategy):
    ...
    def generate_signal(self, data):
        if len(data) < self.window:
            return 0
        mean = data['price'].rolling(window=self.window).mean().iloc[-1]
        std = data['price'].rolling(window=self.window).std().iloc[-1]
        price = data['price'].iloc[-1]
        if price < mean - self.std_dev * std:
            return 1  # 买入信号
        elif price > mean + self.std_dev * std:
            return -1  # 卖出信号
        return 0  # 持有信号
...
class BreakoutStrategy(Strategy):
    ...
    def generate_signal(self, data):
        if len(data) < self.window:
            return 0
        high = data['price'].rolling(window=self.window).max().iloc[-1]
        low = data['price'].rolling(window=self.window).min().iloc[-1]
        price = data['price'].iloc[-1]
        if price > high:
            return 1  # 买入信号
        elif price < low:
...
class MomentumStrategy(Strategy):
    ...
    def generate_signal(self, data):
        if len(data) < self.window:
            return 0
        momentum = data['price'].pct_change(periods=self.window, fill_method=None).iloc[-1]
        if momentum > 0.02:
            return 1  # 买入信号
        elif momentum < -0.02:
            return -1  # 卖出信号
...
class StatisticalArbitrageStrategy(Strategy):
    ...
    def generate_signal(self, data):
        if len(data) < self.window:
            return 0
        mean = data['price'].rolling(window=self.window).mean()
        std = data['price'].rolling(window=self.window).std()
...
class FibonacciRetracementStrategy(Strategy):
    ...
    def generate_signal(self, data):
        ...
        for level in fib_levels:
            if abs(price - level) / level < 0.01:  # 如果价格接近某个斐波那契水平
                if price > level:
                    return 1  # 买入信号
                else:
                    return -1  # 卖出信号
...
class TradingSystem:
    ...
    def preprocess_data(self):
        try:
            # 计算收益率
            self.data['Returns'] = self.data['price'].pct_change(fill_method=None)
            
            # 计算波动率
            self.data['Volatility'] = self.data['Returns'].rolling(window=20, min_periods=1).std() * np.sqrt(252)
            
            # 计算移动平均线
            self.data['MA10'] = self.data['price'].rolling(window=10, min_periods=1).mean()
            self.data['MA30'] = self.data['price'].rolling(window=30, min_periods=1).mean()
            ...
            # 添加 MACD 指标
            self.data['MACD'], self.data['Signal'], _ = talib.MACD(self.data['price'])
    ...
    def select_strategy(self):
        ...
        volatility = recent_data['price'].pct_change(fill_method=None).std()
        trend = recent_data['price'].pct_change(20, fill_method=None).iloc[-1]
        momentum = recent_data['price'].pct_change(10, fill_method=None).iloc[-1]
