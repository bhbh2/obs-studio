...
class TradingSystem:
    ...
    def load_data(self):
        try:
            # 直接指定列名，不使用文件中的标题行
            self.data = pd.read_csv(self.csv_file, names=['datetime', 'price'], parse_dates=['datetime'])
            
            self.data = self.data.sort_values('datetime')
            
            # 确保 start_date 和 end_date 是 Timestamp 对象
    ...
    def preprocess_data(self):
        try:
            ...
            # 添加 high 和 low 列，因为某些指标可能需要这些数据
            self.data['high'] = self.data['price']
            self.data['low'] = self.data['price']
            self.data['close'] = self.data['price']  # 添加 close 列，与 price 相同
    ...
    def select_strategy(self):
        ...
        trend = recent_data['price'].pct_change(20).iloc[-1]
        momentum = recent_data['price'].pct_change(10).iloc[-1]
