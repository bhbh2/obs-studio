...
class TrendFollowingStrategy(Strategy):
    ...
    def generate_signal(self, data):
        ...
        data['MACD'] = exp1 - exp2
        data['Signal_Line'] = data['MACD'].ewm(span=self.macd_signal, adjust=False).mean()
...
class FibonacciRetracementStrategy(Strategy):
    def __init__(self):
        ...
        self.trend_threshold = 0.05  # 5% 趋势阈值

    def generate_signal(self, data):
        if len(data) < self.lookback_period:
            return 0  # 如果数据不足，不生成信号
        ...
        # 判断趋势
        trend = (current_price - data['close'].iloc[-self.lookback_period]) / data['close'].iloc[-self.lookback_period]
        
        fib_levels = [high - (high - low) * level for level in self.fib_levels]
        
        # 上升趋势中的回调
        if trend > self.trend_threshold:
            for level in reversed(fib_levels):
    ...
    def adjust_parameters(self, performance):
        ...
        if performance['profit'] > 0:
            self.tolerance *= 0.95  # 如果盈利，稍微收紧容差
        else:
            self.tolerance *= 1.05  # 如果亏损，稍微放宽容差
        self.tolerance = max(0.005, min(0.02, self.tolerance))  # 保持在 0.5% 到 2% 之间
...
class TradingSystem:
    ...
    def select_strategy(self):
        recent_data = self.data.iloc[-30:]  # 使用最近30个数据点
        ...
        elif abs(trend) > 0.02:  # 如果有明显的趋势，使用斐波那契回调策略
...
class FibonacciRetracementStrategy(Strategy):
    ...
    def generate_signal(self, data):
        ...
        # 如果价格接近某个斐波那契水平，生成信号
        for level in fib_levels:
            if abs(current_price - level) / level < 0.01:  # 1%的容差
        ...
        while True:
            ...
            if price is not None and timestamp is not None:
                ...
                if action:
                    ...
                    print(f"使用策略: {type(self.current_strategy).__name__}")
                if trade_record:
                    print("最新交易记录:")
