...
class MomentumStrategy(Strategy):
    ...
    def generate_signal(self, data):
        momentum = data['close'].pct_change(self.momentum_period, fill_method=None)
...
class TradingSystem:
    ...
    def load_data(self):
        try:
            ...
            # 检查是否有标题行
            has_header = 'datetime' in first_lines[0].lower() or 'price' in first_lines[0].lower()
            
            # 读取CSV文件
            self.data = pd.read_csv(self.csv_file, 
                                    header=0 if has_header else None, 
                                    names=['datetime', 'price'] if not has_header else None)
            ...
            end_date = pd.to_datetime(self.end_date) if self.end_date else None
            
            self.data = self.data[(self.data['datetime'] >= start_date) & 
                                  (self.data['datetime'] <= end_date if end_date else True)]
            
            if self.data.empty:
                logger.warning("警告：在指定的时间范围内没有数据")
                print("警告：在指定的时间范围内没有数据")
            else:
    ...
    def preprocess_data(self):
        try:
            ...
            self.data['MA10'] = self.data['price'].rolling(window=10).mean()
            self.data['MA30'] = self.data['price'].rolling(window=30).mean()
    ...
    def calculate_portfolio_value(self):
        price = self.data['price'].iloc[self.current_index]
    ...
    def update_indicators(self):
        ...
        self.data['MA10'] = self.data['price'].rolling(window=10).mean()
        self.data['MA30'] = self.data['price'].rolling(window=30).mean()
        self.data['RSI'] = talib.RSI(self.data['price'])
...
def main():
    ...
    if choice == '1':
        csv_file = "e:/data/29#CL8.csv"  # 实时交易的默认数据文件
        ...
        csv_file = input("请输入回测CSV文件的路径: ")
        trading_system = TradingSystem(csv_file, initial_balance, start_date, end_date, state_file)
        
        print(f"开始回测CSV文件: {csv_file}")
