    ...
    def real_time_trade(self, timestamp, price):
        try:
            ...
            if stop_loss_take_profit_action:
                ...
                self.total_profit += result['盈利'] if result['盈利'] != "未实现" else 0
...
def main():
    ...
    with open("e:/data/29#CL9.csv", "r", encoding="utf-8") as r:
        next(r)  # 跳过标题行
        for line in r:
            ...
            try:
                ...
                print(f"警告：无法解析价格 '{fields[1]}'，跳过此行")
                continue
            ...
            try:
                ...
                print(f"警告：无法解析日期时间 '{datetime_str}'，跳过此行")
                continue
            
            action, trade_record, total_profit = trading_system.real_time_trade(datetime_obj, price)
            if action:
                print(json.dumps(action, ensure_ascii=False, indent=2))
            if trade_record:
                print("最新交易记录:")
    ...
    print(f"总盈利: ${trading_system.total_profit:.2f}")
