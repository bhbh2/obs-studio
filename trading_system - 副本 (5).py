...
class TradingSystem:
    def __init__(self, csv_file, initial_balance=100000, start_date='2024-09-01', end_date=None):
...
def main():
    ...
    trading_system = TradingSystem(csv_file, initial_balance, start_date, end_date)
    ...
    '''
    # 模拟实时交易
    for i in range(len(trading_system.data)):
        timestamp = trading_system.data['datetime'].iloc[i]
        price = trading_system.data['close'].iloc[i]
    ...
    '''
    r = open("e:/data/29#CL9.csv","r",encoding="utf-8")
    #print(r)
    i = 0
    for line in r.readlines():
        ...
        datetime = first_line[0]
        price = float(first_line[1])
        
        #print(f"{datetime},{price}")
        action, trade_record = trading_system.real_time_trade(datetime, price)
        if action:
            #print(json.dumps(action, ensure_ascii=False, indent=2))
            pass
        if trade_record:
    ...
    for trade in trading_system.trades:
        if len(trade) == 7 and trade[4] is not None:  # 平仓交易
            print(json.dumps({
                ...
                "开仓时间": trade[3].strftime("%Y-%m-%d %H:%M:%S"),
                "平仓价格": round(float(trade[4]), 2),
                "平仓时间": trade[5].strftime("%Y-%m-%d %H:%M:%S"),
    ...
    for trade in trading_system.open_trades:
        print(json.dumps({
            ...
            "开仓时间": trade['time'].strftime("%Y-%m-%d %H:%M:%S"),
            "平仓价格": "未平仓",
            "平仓时间": "未平仓",
            "盈利": "未实现"
        }, ensure_ascii=False, indent=2))
