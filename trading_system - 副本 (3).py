...
class TradingSystem:
    ...
    def real_time_trade(self, timestamp, price):
        try:
            ...
            if stop_loss_take_profit_action:
                ...
                self.save_state()
                return result, self.get_latest_trade_record()
    ...
    def save_state(self):
        state = {
            ...
            'trades': [(t[0], t[1], t[2], 
                        t[3].strftime("%Y-%m-%d %H:%M:%S") if isinstance(t[3], datetime.datetime) else t[3], 
                        t[4], 
                        t[5].strftime("%Y-%m-%d %H:%M:%S") if t[5] and isinstance(t[5], datetime.datetime) else t[5], 
                        t[6]) for t in self.trades],
            'open_trades': [{**trade, 'time': trade['time'].strftime("%Y-%m-%d %H:%M:%S") if isinstance(trade['time'], datetime.datetime) else trade['time']} 
                            for trade in self.open_trades],
            'current_index': self.current_index,
            'last_trade_index': self.last_trade_index,
            'ma_short': list(self.ma_short),
            'ma_long': list(self.ma_long)
        }
    ...
    def load_state(self):
        if os.path.exists(self.state_file):
            ...
            self.trades = [(t[0], t[1], float(t[2]), 
                            datetime.datetime.strptime(t[3], "%Y-%m-%d %H:%M:%S") if t[3] else None, 
                            float(t[4]) if t[4] is not None else None, 
                            datetime.datetime.strptime(t[5], "%Y-%m-%d %H:%M:%S") if t[5] else None, 
                            float(t[6]) if t[6] is not None else None) for t in state['trades']]
            self.open_trades = state['open_trades']
            self.current_index = state['current_index']
            self.last_trade_index = state['last_trade_index']
            self.ma_short = deque(state['ma_short'], maxlen=10)
            self.ma_long = deque(state['ma_long'], maxlen=30)
        else:
            # 初始化状态
            pass
...
def main():
    ...
    for i in range(trading_system.current_index, len(trading_system.data)):
        ...
        action, trade_record = trading_system.real_time_trade(timestamp, price)
        if action:
            print(json.dumps(action, ensure_ascii=False, indent=2))
        if trade_record:
            print("最新交易记录:")
    ...
    # 在循环结束后，打印总结信息
    print("\n交易模拟完成")
    print(f"最终资金: ${trading_system.balance:.2f}")
    print(f"总交易次数: {len(trading_system.trades)}")
    
    # 打印所有交易记录
    print("\n所有交易记录:")
    for trade in trading_system.trades:
        if len(trade) == 7 and trade[4] is not None:  # 平仓交易
            print(json.dumps({
                "交易类型": trade[0],
                "手数": trade[1],
                "开仓价格": round(float(trade[2]), 2),
                "开仓时间": trade[3],
                "平仓价格": round(float(trade[4]), 2),
                "平仓时间": trade[5],
                "盈利": round(float(trade[6]), 2)
    ...
    # 打印未平仓交易
    print("\n未平仓交易:")
    for trade in trading_system.open_trades:
        print(json.dumps({
            "交易类型": "多" if trade['size'] > 0 else "空",
            "手数": abs(int(trade['size'])),
            "开仓价格": round(float(trade['price']), 2),
            "开仓时间": trade['time'],
            "平仓价格": "未平仓",
            "平仓时间": "未平仓",
            "盈利": "未实现"
